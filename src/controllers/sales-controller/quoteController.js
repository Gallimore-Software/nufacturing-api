...
exports.getAllQuotes = async (req, res) => {
  try {
    const quotes = await Quote.find().populate('accountManager productType formula ingredients');
...
exports.getQuoteById = async (req, res) => {
  try {
    const quote = await Quote.findById(req.params.quoteId).populate('accountManager productType formula ingredients');
    if (!quote) {
...
exports.updateQuote = async (req, res) => {
  try {
    const updatedQuote = await Quote.findByIdAndUpdate(req.params.quoteId, req.body, {
      new: true,
      runValidators: true
    });
    if (!updatedQuote) {
