...
class MongooseInventoryRepository {
    ...
    static async findByNameOrSKU(name, sku) {
        return await inventoryModel_1.InventoryItem.findOne({ $or: [{ name }, { sku }] });
    }
    ...
    static async findById(id) {
        return await inventoryModel_1.InventoryItem.findById(id);
    }
    // Find all inventory items (with optional filtering)
    static async findAll(filter = {}) {
        return await inventoryModel_1.InventoryItem.find(filter);
    }
    // Update inventory item by ID
    static async updateById(id, updateData) {
        return await inventoryModel_1.InventoryItem.findByIdAndUpdate(id, updateData, { new: true });
    }
    // Delete inventory item by ID
    static async deleteById(id) {
        return await inventoryModel_1.InventoryItem.findByIdAndDelete(id);
    }
    // Find inventory items by date range (for tracking inventory over time)
    static async findByDateRange(startDate, endDate) {
        return await inventoryModel_1.InventoryItem.find({
            createdAt: {
                $gte: startDate,
                $lte: endDate,
            },
    ...
    static async combineByAttribute(attribute) {
        return await inventoryModel_1.InventoryItem.aggregate([
            {
                $group: {
                    _id: `$${attribute}`,
                    items: { $push: "$$ROOT" },
    ...
    static async updateStockQuantity(id, quantityChange) {
        return await inventoryModel_1.InventoryItem.findByIdAndUpdate(id, { $inc: { stockQuantity: quantityChange } }, { new: true });
    }
    // Update price information
    static async updatePrice(id, newPrice) {
        return await inventoryModel_1.InventoryItem.findByIdAndUpdate(id, { price: newPrice, priceLastUpdated: new Date() }, { new: true });
    }
    // Mark inventory as processed (e.g., for batch completion)
    static async markAsProcessed(id) {
        return await inventoryModel_1.InventoryItem.findByIdAndUpdate(id, { status: "processed" }, { new: true });
    }
}
